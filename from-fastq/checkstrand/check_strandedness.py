...
def main():
    ...
    # DW Version 2025-01-14
    # DW result = pd.read_csv(test_folder + '/' + 'strandedness_check.txt', sep="\n", header=None)

    # File path
    file_path = test_folder + '/' + 'strandedness_check.txt'
    ...
    for i, line in enumerate(lines):
        if i == 1:  # Fraction of reads failed to determine
            failed = float(line.replace('Fraction of reads failed to determine: ', ''))
        elif i == 2:  # Fraction of reads explained by "1++,1--,2+-,2-+"
            fwd = float(line.replace('Fraction of reads explained by "1++,1--,2+-,2-+": ', ''))
        elif i == 3:  # Fraction of reads explained by "1+-,1-+,2++,2--"
            rev = float(line.replace('Fraction of reads explained by "1+-,1-+,2++,2--": ', ''))

    # Calculate percentages
    total_explainable = fwd + rev if fwd is not None and rev is not None else 1  # Avoid division by zero
    fwd_percent = fwd / total_explainable
    rev_percent = rev / total_explainable

    # Print summary
    print(lines[0])  # "This is PairEnd Data"
    print(lines[1])  # Fraction of reads failed to determine
    print(f"{lines[2]} ({round(fwd_percent * 100, 1)}% of explainable reads)")
    print(f"{lines[3]} ({round(rev_percent * 100, 1)}% of explainable reads)")

    # Check for warnings or comments based on percentages
    if failed > 0.50:
    ...
    if fwd_percent > 0.9:
        ...
        print('== Data is likely FR/fr-secondstrand or forward-stranded')
    elif rev_percent > 0.9:
        print('== Over 90% of reads explained by "1+-,1-+,2++,2--"')
        print('== Data is likely RF/fr-firststrand or reverse-stranded')
